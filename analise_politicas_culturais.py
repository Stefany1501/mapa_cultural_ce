...
XLSX_PATH = "adesao_aldir.xlsx"
CSV_MUN_LPG = "adesaomunicipioslpg.csv"
CSV_EST_LPG = "adesaoestadoslpg.csv"
...
def carregar_dados_mapa_cultural():
    ...
    df_agentes = processar_dataframe_mapa(buscar_dados_paginados(f"{API_BASE}/agent/find", {"@select": "id,name,shortDescription,longDescription,createTimestamp", "@limit": 500}, "Coletando Perfis de Agentes Culturais..."), "ano")
...
# extracao leis
@st.cache_data(ttl=3600, show_spinner=False)
def buscar_projetos_salic(ano_projeto=2021):
    todos_projetos = []
    ...
    while True:
        params = {"limit": limit, "offset": offset, "uf": "CE"}
    ...
    for p in todos_projetos:
        ...
        municipios_ce = []
        vinculado_ao_ce = False
        
        if str(p.get("UF", "")).upper() == "CE": vinculado_ao_ce = True

        if isinstance(locais, list):
            for local in locais:
                uf_local = str(local.get("UF", local.get("uf", ""))).upper()
                if uf_local == "CE":
                    vinculado_ao_ce = True
                    muni = local.get("municipio")
                    if muni and muni not in municipios_ce: municipios_ce.append(muni)
        
        if not vinculado_ao_ce: continue
        municipio_execucao = ", ".join(municipios_ce) if municipios_ce else p.get("municipio", "Ceará (Geral)")
...
def carregar_lei_paulo_gustavo(caminho_mun: str, caminho_est: str):
    """Carrega os arquivos CSV da Lei Paulo Gustavo filtrados para o Ceará."""
    df_mun = pd.read_csv(caminho_mun)
    df_est = pd.read_csv(caminho_est)
    
    df_mun_ce = df_mun[df_mun["UF"] == "CE"].copy()
    df_est_ce = df_est[df_est["Estado"] == "CEARA"].copy()
    
    df_mun_ce["Valor Disponível"] = pd.to_numeric(df_mun_ce["Valor Disponível"], errors="coerce").fillna(0)
    df_est_ce["Valor Disponível"] = pd.to_numeric(df_est_ce["Valor Disponível"], errors="coerce").fillna(0)
    
    return df_mun_ce, df_est_ce
...
# definindo abas
tab1, tab2, tab3, tab4 = st.tabs(["|Análise de Editais e Entidades|", "|Lei Rouanet (SALIC)|", "|Lei Aldir Blanc (PNAB)|", "|Lei Paulo Gustavo|"])
...
with tab2:
    ...
    ano_selecionado = st.selectbox("Filtrar Ano de Execução do Projeto:", options=anos_disponiveis, index=anos_disponiveis.index(2021))
    
    df_salic = buscar_projetos_salic(ano_projeto=ano_selecionado)
...
with tab4:
    ...
    try:
        df_mun_lpg_ce, df_est_lpg_ce = carregar_lei_paulo_gustavo(CSV_MUN_LPG, CSV_EST_LPG)
